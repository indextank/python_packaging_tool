    ...
    def get_hidden_imports(self) -> List[str]:
        ...
        for dep in self.dependencies:
            ...
            is_configured = (
                ...
                dep_normalized in self.CONFIGURED_LIBRARIES or
                module_name in self.CONFIGURED_LIBRARIES or
                module_lower in {lib.lower() for lib in self.CONFIGURED_LIBRARIES}
            )
        ...
        if unconfigured:
            ...
            for dep in sorted(unconfigured):
                ...
                # 将 PyPI 包名转换为实际模块名
                module_name = self.PACKAGE_TO_MODULE_MAPPING.get(dep)
                ...
                # 检测是否是真正的包还是单文件模块
                is_package = self.is_real_package(module_name)

                if is_package:
                    self.log(f"  ⚠️ {dep} -> {module_name} (包)")
                    # 如果已经自动收集了子模块，使用它们
                    ...
                    elif module_name in self._auto_collected_modules:
                    ...
                    # 单文件模块，不需要添加子模块
                    if dep != module_name:
                        self.log(f"  ⚠️ {dep} -> {module_name} (单文件模块)")
                    else:
...
'''
    ...
    def auto_collect_submodules(
        ...
        # 将 PyPI 包名转换为实际模块名
        module_name = self.PACKAGE_TO_MODULE_MAPPING.get(package_name, package_name)
